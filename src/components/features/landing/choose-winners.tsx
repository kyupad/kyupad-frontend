      ...
      {width > 600 ? (
        <div className="flex gap-4 flex-col md:flex-row xl:gap-24 px-4 justify-center sm:my-6 lg:my-16">
          {data.map((item, index) => (
            <div
              key={index}
              className="flex hover:bg-kyu-color-2 duration-500 hover:shadow-sm relative shadow-[19px_18px_0px_0px_rgba(0,0,0,0.1)] flex-col items-start sm:gap-4 md:w-[30%] xl:w-[372px] h-72 rounded-xl p-5 border-4 border-kyu-color-11 bg-kyu-color-1 "
            >
              {index < data.length - 1 ? (
                <Image
                  src={vector}
